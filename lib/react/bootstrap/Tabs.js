    ...
    value: function render() {
      ...
      var children = _lodash2.default.flatten(this.props.children).map(function (child) {
        ...
        var _child$props = _extends({}, child.props),
            ...
            childProps = _objectWithoutProperties(_child$props, ['eventKey', 'href', 'render', 'title']);

        var href = hrefProp;

        childProps.key = eventKey;

        var className = childProps.className ? childProps.className : '';
        delete childProps.className;

        if (href) {
          if ((0, _fn.compareURL)(currentUrl, href)) {
